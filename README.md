...
首次使用微信通道时，可以保持 `allowUsers` 为空。扫码登录成功后，Bridge 会自动把本次登录微信账号对应的 iLink 用户 ID 写入配置文件的 `allowUsers`，后续该用户发来的消息会直接执行 Agent。
...
注意：这个 ID 不是微信号，也不是昵称，而是 iLink 用户 ID。其他用户或其他通道仍需手动配置 `allowUsers`。
